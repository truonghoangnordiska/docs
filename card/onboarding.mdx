...
flowchart LR
    ...
    PROCESSING --> |Approved| AWAITING_SIGNING
    PROCESSING --> |Failed credit assessment| REJECTED
    PROCESSING --> |Retryable error| FAILED
    AWAITING_SIGNING --> |Signed| AWAITING_CARD_CREATION
    AWAITING_SIGNING --> |Retryable error| FAILED
    AWAITING_CARD_CREATION --> |Card created| COMPLETED
    AWAITING_CARD_CREATION --> |Retryable error| FAILED
    COMPLETED --> End1([End])
    REJECTED --> End2([End])
    FAILED --> End3([End])
```